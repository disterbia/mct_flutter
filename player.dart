...
List<double> NOW_TRACK_DURATION = [];
        ...
        parsedMidiTracks.forEach((midiTrack) {
          ...
          midiTrack.forEach((element) {
            ...
            if (element is NoteOffEvent) {
              ...
              durations.add(tem);
              if (recorderLineIndex == trackIndex) {
                NOW_TRACK_DURATION.add(element.deltaTime == 0
                    ? durations[durations.length - 1]
                    : element.deltaTime);
              }
            }
...
List<Note> midiNoteToNote(
  ...
  midiNote.forEach((note) {
    ...
    if (position <= note.startPosition && position > note.startPosition - 20) {
      ...
      int restLoop = restNotes.length;

      if (restLength > 0) {
        for (int i = 0; i < restLoop; i++) {
          if (maxLengthOfMadi >= restNotes[i] &&
              nowMadiLength + restNotes[i] <= maxLengthOfMadi) {
            int cnt = (restLength / restNotes[i]).floor();
            if (cnt > 0) {
              restLength -= restNotes[i];
              notes.add(Note(
                  leng: restNotes[i], pitch: -1, state: 0, scale: note.scale));
              nowMadiLength = nowMadiLength + restNotes[i] >= maxLengthOfMadi
                  ? 0
                  : nowMadiLength + restNotes[i];
              if (nowMadiLength == 0) {
                i = 0;
              }
              i--;
